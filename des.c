...
{
    state[0] =  BITCON(in[0],in[1],in[2],in[3]);    //L

    state[1] =  BITCON(in[4],in[5],in[6],in[7]);    //R
}

void InvIP(uint state[], uchar in[])
{
    in[0] = (state[0] >> 24) & 0x000000ff;

    in[1] = (state[0] >> 16) & 0x000000ff;

    in[2] = (state[0] >> 8) & 0x000000ff;
    
    in[3] = (state[0])       & 0x000000ff;

    in[4] = (state[1] >> 24) & 0x000000ff;
    
    in[5] = (state[1] >> 16) & 0x000000ff;
    
    in[6] = (state[1] >> 8) & 0x000000ff;
    
    in[7] = (state[1])       & 0x000000ff;
}
